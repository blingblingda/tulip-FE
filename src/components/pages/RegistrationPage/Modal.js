...
function PassionsModal() {
    ...
    const togglePassion = (passion) => {
        if (selectedPassions.some(p => p.label === passion.label)) {
            setSelectedPassions(prev => prev.filter(p => p.label !== passion.label));
        } else {
    ...
    return (
        <div>
            ...
            {savedPassions.length > 0 && (
                <div className="mt-4">
                    You are passionate about: {savedPassions.map(p => `${p.emoji} ${p.label}`).join(", ")}
                </div>
            )}
            ...
            {isModalOpen && (
                <div className="fixed z-10 inset-0 overflow-y-auto" onClick={() => setModalOpen(false)}>
                    <div className="flex items-center justify-center min-h-screen text-center sm:block sm:p-0">
                        ...
                        >
                            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <h2 className="text-xl font-semibold mb-4">Select Your Passions (max 5)</h2>
                                        ...
                                        <div key={passion.label}>
                                            <label className="flex items-center">
                                                <input
                                                    ...
                                                    checked={selectedPassions.some(p => p.label === passion.label)}
                                                    onChange={() => togglePassion(passion)}
                                                />
                                                <span className="ml-2">{passion.emoji} {passion.label}</span>
                                            </label>
                                        </div>
                                    ))}
                                </div>
